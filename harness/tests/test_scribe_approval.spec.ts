...
describe('scribe-approval gate', () => {
  ...
  it('asks with patient, encounter, section, exact diff and source quotes, then applies exactly once', async () => {
    ...
    expect(prompt).toContain('+ Douleur à la palpation du LTFA droit.')
    expect(prompt).toContain('  Paramètres à l\'arrivée : TA 124/78 mmHg')
  ...
  it('record_restore is asked for, relayed and audited the same way', async () => {
    ...
    expect(h.asked[1]!.displayReason!.en).toContain('- Douleur à la palpation du LTFA droit.')
