...
## Amendment 1 (2026-09-25, after senior consultation with Astra; ruling by Fable)

Astra's review of this plan (`.superpowers/sdd/astra/plan-w3/avis.md`) found the approval boundary in the wrong layer. Accepted in full; the tasks below override the earlier text where they conflict.

**A. The bridge owns approval, not the harness.** `record_apply` and `record_restore` on the bridge require an **approval token** that only the bridge can mint and only after a human decision. The token binds `{patient_id, encounter_id, item_id, attribute, mode, base_digest, final_text_digest, draft_id}`, expires (10 min), is single-use, and is stored bridge-side; the model never sees it. Flow: plugin `record_draft_edit` → bridge stores the draft immutably and returns `draft_id` → the dsh approval prompt (H4) shows patient, encounter, the exact diff and the source quotes → on `allow`, the **approval plugin** (not the model) calls bridge `approve_draft(draft_id, answerer)` over a separate loopback credential (`PORTAIL_BRIDGE_APPROVER_TOKEN`, distinct from the tool-side `PORTAIL_BRIDGE_TOKEN`) → the bridge mints the approval token internally and marks the draft `approved` → `record_apply(draft_id)` from the tool side succeeds only if the draft is `approved`, its base digest still matches the live section, and the token is unconsumed; the bridge performs digest check + write + token consumption **atomically** (single lock, journal line written before the write, idempotent replay by `draft_id`). The dsh `tools/pre-execute` gate remains as UX (it is what asks the human), the `guard()` stays as belt-and-braces, but neither is the security boundary.

**B. Drafts live in the bridge.** `harness/bridge/drafts.jsonl` (append-only): `{draft_id, ts, patient_id, encounter_id, item_id, attribute, mode, base_digest, new_text, rationale, quotes:[{dictation_id, offset, text}], status: drafted|approved|applied|rejected|expired, approval:{id, ts, answerer}, apply:{ts, version_before, version_after, backup_id}}`. Quotes must reference a stored dictation id + character offsets; the bridge validates that each quoted text is a substring of that dictation (fetched from the VoxLocal API or the seed file) and rejects the draft otherwise ("citation introuvable dans la dictée").

**C. Patient binding.** Every dictation record carries a `patient_context` (set by the clinician in the chat before dictating, or by a `patient_set` tool the model may only call with the clinician's explicit instruction); a draft's `patient_id` must equal the dictation's; the approval prompt shows both. A mismatch is a bridge error, not a model judgement.

**D. Real-time semantics (H2, H2b).** H2 delivers completed dictations via `agent.inject()` (durable context, no turn) and triggers `followup()` only on a pause ≥ 20 s, on an explicit clinician command, or on a dictation marked "fin". H2b (new, after H2): VoxLocal streams **stabilised segments** (every ~30 s of audio, cleaned) through `GET /v1/dictations/<id>/segments?since=` so the agent can prepare during speech; until H2b lands, "temps réel" means "per completed dictation" and the docs say so.

**Decisive tests for H4 (mandatory):** direct bridge `apply` without approval → 403; apply after the draft text changed → 409; apply after the live section changed (digest) → 409; apply with a token bound to another patient → 403; replay of an applied `draft_id` after bridge restart → idempotent (no second write, same result); `record_restore` also token-gated.
